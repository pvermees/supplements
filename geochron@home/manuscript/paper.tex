...
where the students' pooled ratio differs somewhat from PV's.  The
logcontrast between the student ratios and PV's ratios for all 300
pairs of grains follows a normal distribution with a mean of 0.0 and
standard deviation 0.23 (Table~\ref{tab:logcontrasts}). This indicates
that, on the whole, the student estimates of the (log)ratios are
unbiassed with respect to the expert opinion of PV.
\medskip
...
\appendix
\section{Summary tables for the crowd-sourcing experiment}
...
\begingroup
\captionof{table}{
  Logcontrasts between the pooled count ratio of the
  students and PV for each pair of 25 grains in
  Table~\ref{tab:crowdtable}.
}
\label{tab:logcontrasts}\medskip
\renewcommand{\arraystretch}{0.7}
\begin{scriptsize}
  \input{../output/logcontrasts.txt}
\end{scriptsize}
\endgroup
