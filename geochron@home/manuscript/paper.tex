...
{ \centering \includegraphics[width=\linewidth]{radialcrowd.pdf}
  \captionof{figure}{a) box plots of 25 crowd-sourced sets of fission
    track counts for 69 analysts; b) comparison of grains~23 and 25,
    which were chosen because of their high counts and variability; c)
    radial plot of the pairwise ratios of the fission track counts for
    grains~23 and 25. The results of analyst PV are shown in
    blue.\medskip}
...
course in isotope geology. 68~students were asked to create an account
...
Section~\ref{sec:private}. The majority of the student analysts
undercounted the fission tracks by up to a factor of five compared to
the `expert' analyst (PV, shown in blue). The poor consistency of the
results reflects the very limited training received by the
students.\medskip

Grains 23 and 25 have greater than average track densities, and are
particularly dispersed. Plotting the 37 paired counts for each user on
a scatter plot reveals a strong correlation
(Figure~\ref{fig:radialcrowd}b). This means that students who over- or
undercount the first grain, also tend to over- or undercount the
second grain. Even though most students count fewer fission tracks
than PV, the ratio of their counts are similar.
Figure~\ref{fig:radialcrowd}c compares grains 23 and 25 on a radial
plot, where
\begin{equation}
...
The tight clustering of the data within the 2-se band confirms that
the pairwise ratios of the counts for each analyst are more consistent
than the raw counts of Figure~\ref{fig:crowdsourcing}a.  Despite the
tremendous scatter of the raw track counts obtained by the citizen
scientists, their pairwise ratio estimates scatter evenly around the
expert opinion. This suggests that the collective wisdom of the entire
